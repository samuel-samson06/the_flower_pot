...
function ProductsDisplay({data,sliceStart=0,sliceEnd}) {
  return (
    <React.Fragment>
        <div className=' py-5 px-2 justify-center gap-5 sm:gap-7 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 '>
            {
                    ...
                    return (
                        <NavLink key={eachData.id} to={`/${eachData.id}`}>
                            <div  className=' border-[1px] border-[#5f5d5d] shadow-sm rounded-md space-y-2 w-fit  flex flex-col bg-[#eaeaea]'>
                                <img src={eachData.image} className=' border-b-[1px] border-black size-[200px] sm:size-[230px] rounded' alt={eachData.title} />
